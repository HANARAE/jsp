...
/*
 - 서블릿이란 웹 페이지를 자바 언어로만 구성하는 것이다. 즉, jsp 파일을 자바 언어로만 구성하는 것이다. (...con.jsp)
 톰캣을 사용해서 jsp 파일을 자동으로 class로 변환했다면 이제는 직접 클래스를 생성해서 클래스로 요청을 처리하자.
*/
...
public class ServletBasic extends HttpServlet {
	...
	public void init() throws ServletException {
		/*
		 ...
		 - init()은 컨테이너(서버 프로그램)에 의해 서블릿 객체가 생성될 때 최초 1회 자동으로 호출됨.
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//응답 화면 제작
		//클래스에서 브라우저로 바로 응답을 구현하기 위해 PrintWriter객체를 사용.
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//매개값으로 내장객체 request와 response가 전달되므로 객체의 메서드를 통해 파라미터값 가져오거나, 페이지 이동이 가능하다
