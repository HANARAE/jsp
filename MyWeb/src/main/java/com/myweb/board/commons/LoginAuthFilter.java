...
public class LoginAuthFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		/*
		 ...
		 - request객체와 resonse객체를 넘겨주기 때문에 이를 가지고 요청과 응답을 조작할 수 있다.
		 - 그리고 FilterChain을 통해서 조작 이후 요청을 원래 목적지인 서블릿으로 전달할 수 있다.
		 */
		System.out.println("doFilter 메서드 발동");
		
		//우리가 평소에 사용하던 타입은 HttpServletRequest이다.
		//부모 타입의 ServletRequest를 자식 타입으로 끌어내리면 사용 가능하다
		HttpSession session = ((HttpServletRequest)request).getSession();
		if(session.getAttribute("user") == null) { //로그인이 안 돼 있는 상태
			System.out.println("회원 권한 없음. 통과 안 됨");
		...
		chain.doFilter(request, response); //로그인이 돼 있다면
	}
